...
file_path2='C:/Users/ALEX/Desktop/DataMining/Proj/data/test.txt'
...
with open('C:/Users/ALEX/Desktop/DataMining/Proj/cna.cbow.cwe_p.tar_g.512d.0.txt', encoding="utf-8") as f:
    for line in f:
...
def newToken(data_list, embedding_list, dim):
    new_embedding_list = list()
    for i in range(len(data_list)):
        
        
        idx_emb = 0
        #int(len(embedding_list[i])/100)+1 => len(embedding_list[i])//100+1  
        while idx_emb < len(embedding_list[i]):
            embedding_list_tmp = list()
            if len(embedding_list[i]) - idx_emb < 100:                
                for l in range(len(embedding_list[i])%100):
                    embedding_list_tmp.append(embedding_list[i][idx_emb])
                    idx_emb = idx_emb + 1  
                for l in range(100-len(embedding_list[i])%100):
                    #listofzeros = [0] * dim
                    #embedding_list_tmp.append(listofzeros)   
                    embedding_list_tmp.append(np.zeros(dim))
                    idx_emb = idx_emb + 1  
                ...
                for k in range(100): 
                    embedding_list_tmp.append(embedding_list[i][idx_emb])
                    idx_emb = idx_emb + 1       
            new_embedding_list.append(embedding_list_tmp)
...
def Feature(embed_list, dim):
    feature_list = list()
    for idx_list in range(len(embed_list)):
        feature_list_tmp = list()
        for idx_tuple in range(len(embed_list[idx_list])):
            feature_dict = dict()
            for idx_vec in range(dim):
...
def newToken_label(data_list, embedding_list):
    new_embedding_list = list()
    for i in range(len(data_list)):      
        idx_emb = 0         
        while idx_emb < len(embedding_list[i]):
            ...
            if len(embedding_list[i]) - idx_emb < 100:                
                ...
                for l in range(100-len(embedding_list[i])%100):
                    output_str =  'O'   
                    embedding_list_tmp.append(output_str)
                    idx_emb = idx_emb + 1  
            else:                
                for k in range(100): 
                    embedding_list_tmp.append(embedding_list[i][idx_emb])
...
x_train_o = newToken(traindata_list, trainembed_list, dim)
x_train = Feature(x_train_o, dim)
y_train_o = Preprocess(traindata_list)
y_train = newToken_label(traindata_list, y_train_o)


# CRF - Test Data (Golden Standard)
x_test_o = newToken(testdata_list, testembed_list, dim)
x_test = Feature(x_test_o, dim)
y_test_o = Preprocess(testdata_list)
y_test = newToken_label(testdata_list, y_test_o)
...
#x_test_upload_o = newToken(testing_list, testing_list, dim)
#x_test_upload = Feature(x_test_upload_o, dim)
x_test_upload = Feature(testing_list, dim)
...
for test_id in range(len(y_pred_upload)):
    ...
    entity_text = None
    entity_type = None
    for pred_id in range(len(y_pred_upload[test_id])-1):
        if y_pred_upload[test_id][pred_id][0] == 'B':
            start_pos = pos
            entity_type = y_pred_upload[test_id][pred_id][2:]
        elif start_pos is not None and y_pred_upload[test_id][pred_id][0] == 'I' and y_pred_upload[test_id][pred_id+1][0]=='I':
            end_pos = pos+1
            entity_text = ''.join(
                [uploadtest_data_text[test_id][position][0] for position in range(start_pos, end_pos)])
...
output_path = 'output_1214_256D_seg100.tsv'
